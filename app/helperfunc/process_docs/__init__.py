...
def main(myblob: func.InputStream):
    ...
    process_document(myblob)
    
def process_document(myblob):
    account_url = os.environ.get("AZURE_BLOB_URL")
    default_credential = DefaultAzureCredential()

    tempFilePath = tempfile.gettempdir()  
    tempFile = tempFilePath + '/' + myblob.name.replace('datadrop/', '')
    ...
    upload_blobs(tempFile, default_credential)
...
def upload_blobs(filename, credential):
    logging.info(f"upload_blobs starting")
    blob_service = BlobServiceClient(account_url=f"https://{os.environ.get('AZURE_BLOBSVC_NAME')}.blob.core.windows.net", credential=credential)
    blob_container = blob_service.get_container_client(os.environ.get('TARGET_CONTAINER'))
    logging.info(f"Container create")
...
def get_document_text(filename):
    ...
    form_recognizer_client = DocumentAnalysisClient(endpoint=f"https://{os.environ.get('FORM_REC_SERVICE')}.cognitiveservices.azure.com/", credential=DefaultAzureCredential(), headers={"x-ms-useragent": "azure-search-chat-demo/1.0.0"})
